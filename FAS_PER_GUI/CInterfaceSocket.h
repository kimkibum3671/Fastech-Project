...
class CInterfaceSocket : protected CInterface
{
protected:
	CInterfaceSocket();
...
public:
	virtual const BOOL	Connect(IN const DWORD dwAddress, IN const INT nPortNo);
	virtual const BOOL	Close(IN const DWORD dwAddress);
...
//	virtual const BOOL	ChangeMotiongateName(IN const INT iID, IN LPCTSTR lpszName);
	...
	virtual const BOOL	SetParameter(IN const INT iID, const INT iAxisNo, const INT nParameterIndex, const LONG lValue);
	virtual const BOOL	GetParameter(IN const INT iID, const INT iAxisNo, const INT nParameterIndex, LONG* pValue);
	virtual const BOOL	GetFlashParameter(IN const INT iID, const INT iAxisNo, const INT nParameterIndex, LONG* pValue);
	virtual const BOOL	SetPositionTableItem(IN const INT iID, const INT iAxisNo, const INT nTableIndex, IN LPCVOID pbtItem, IN const INT cbNodeSize);
	virtual const BOOL	GetPositionTableItem(IN const INT iID, IN const INT iAxisNo, IN const INT nTableIndex, OUT LPVOID pbtItem, IN const INT cbNodeSize);
	virtual const BOOL	SaveAllParameters(IN const INT iID, IN const INT iAxisNo);
	virtual const BOOL	SavePositionTableItems(IN const INT iID, IN const INT iAxisNo);
	virtual const BOOL	LoadPositionTableItems(IN const INT iID, IN const INT iAxisNo);
	...
	virtual const BOOL	GetMotionGateAlarmStatus(IN const INT iID, OUT BYTE* pStatus);
