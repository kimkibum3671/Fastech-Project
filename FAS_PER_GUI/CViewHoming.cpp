...
IMPLEMENT_DYNCREATE(CViewHoming, CViewDevice)

CViewHoming::CViewHoming()
	: CViewDevice(IDD_HOMING)
{
	m_nClickItemID = \
...
void CViewHoming::DoDataExchange(CDataExchange* pDX)
{
	CFormView::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CViewHoming, CFormView)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BTN_ADMIN, &CViewHoming::OnBnClickedBtnAdmin)
	ON_BN_CLICKED(IDC_BTN_RESET, &CViewHoming::OnBnClickedBtnReset)
	ON_BN_CLICKED(IDC_BTN_ESTOP, &CViewHoming::OnBnClickedBtnEstop)
	ON_BN_CLICKED(IDC_BTN_STOP, &CViewHoming::OnBnClickedBtnStop)
	ON_BN_CLICKED(IDC_BTN_SEL_ALL, &CViewHoming::OnBnClickedBtnSelAll)
	ON_BN_CLICKED(IDC_BTN_SEL_NOT, &CViewHoming::OnBnClickedBtnSelNot)
END_MESSAGE_MAP()

// CViewHoming diagnostics

#ifdef _DEBUG
void CViewHoming::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CViewHoming::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CViewHoming message handlers


void CViewHoming::OnInitialUpdate()
{
...
void CViewHoming::OnTimer(UINT_PTR nIDEvent)
{
	switch (nIDEvent)
	{
...
const DWORD CViewHoming::GetSelectedMotorIndex()
{
	DWORD dwResult = 0;
	const INT nResID[] = {
		IDC_BTN_CHECK1,
...
void CViewHoming::DisplayMotionGateStatus(CDocument* pDoc)
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	if (NULL == pDevice)
	{
...
void CViewHoming::DisplayMotorStatus(CDocument* pDoc)
{
...
void CViewHoming::DisplayName(CDeviceMotionGate* pDevice)
{
...
BOOL CViewHoming::PreTranslateMessage(MSG* pMsg)
{
	...
	return CViewDevice::PreTranslateMessage(pMsg);
}

void CViewHoming::EnableControls(CDocument* pDoc)
{
	EnableMotorSelection(pDoc);
	EnableReset(pDoc);
	EnableNormalMotionControl(pDoc);
	EnableStopMotionControl(pDoc);
}

void CViewHoming::EnableMotorSelection(CDocument* pDoc)
{
...
void CViewHoming::EnableReset(CDocument* pDoc)
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	BOOL bControlable = FALSE;
	switch (pDevice->GetOpCode())
...
void CViewHoming::EnableNormalMotionControl(CDocument* pDoc)
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	BOOL bControlable = FALSE;
	switch (pDevice->GetOpCode())
...
void CViewHoming::EnableStopMotionControl(CDocument* pDoc)
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	BOOL bControlable = FALSE;
	switch (pDevice->GetOpCode())
...
void CViewHoming::ShowControls(const BOOL bEnable)
{
	const INT nResID[] = {
		IDC_ST_GROUP1,
		IDC_BTN_SEL_ALL,
...
void CViewHoming::OnBnClickedBtnAdmin()
{
...
void CViewHoming::OnBnClickedBtnReset()
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	if (pDevice)
	{
...
void CViewHoming::OnBnClickedBtnEstop()
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	if (pDevice)
	{
...
void CViewHoming::OnBnClickedBtnStop()
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	if (pDevice)
	{
...
void CViewHoming::OnBnClickedBtnSelAll()
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	const INT nResID[] = {
		IDC_BTN_CHECK1,
...
void CViewHoming::OnBnClickedBtnSelNot()
{
	CDeviceMotionGate* pDevice = GetMotiongateCtrl();
	const INT nResID[] = {
		IDC_BTN_CHECK1,
...
void CViewHoming::CheckHomingResult()
{
	if (FALSE == m_bHoming)
	{
		return;
...
void CViewHoming::ShowMotorInformation(const INT nIndex, const INT nCmdShow)
{
	const INT nResID[][5] = {
